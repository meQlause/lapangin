...
Numbers **inside a Zod schema** are the schema's own constants
(`z.string().min(1)`, `z.number().int().min(1024)`) — they read naturally in
place and are the `const`. Applying the rule inside a schema is noise, so
keep it in the service-and-rules glob and leave `*.schema.ts` on the default.
