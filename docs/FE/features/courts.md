...
| `data.filters.startTime`, `.duration`, `.sport` | Echoed into the `EmptyState` copy (`Sabtu, 27 September 2026 jam 10:00 selama 2 jam`) | FR-016 |
...
export const courtsFilterSchema = z.object({
  ...
  startTime: z.string().regex(/^([01]\d|2[0-3]):(00|30)$/).default('06:00'), // FR-011
...
| `sport` | One of `futsal`, `badminton`, `basket`, or absent | Same. **Absent means "all sports"** — `Semua` is the *absence* of the parameter, not the literal `"Semua"` ([`../../BE/features/courts.md`](../../BE/features/courts.md) §1 notes) | FR-013 |
...
| `Tidak tersedia` button (maintenance court) | Disabled; renders as `<button disabled>`, focusable and read by screen readers | The card still lists amenities and the maintenance line so the user understands *why* (FR-006) |
...
- **Given** the same, **when** the page loads, **then** the Futsal A availability strip renders eight chips in order `06:00`, `07:00`, `08:00`, `09:00`, `10:00`, `11:00`, `19:00`, `20:00`, with `08:00`, `09:00`, `19:00` in `taken` state (line-through) and the rest `free`. (FR-007, FR-008)
- **Given** the same, **when** the page loads, **then** the Badminton 3 strip includes an `11:00` chip in `closed` state with title `Di luar jam operasional`, distinct from a `taken` chip. (FR-008, FR-009)
...
- **Given** the same, **when** the user clicks `Pilih jadwal` on the Futsal A card, **then** the URL becomes `/v/gor-kemang/courts/c1a7f2d4-2b88-4f1a-9a10-0f6e8c11b201/review?date=2026-09-27&startTime=10:00&duration=2`. (FR-014)
...
- **Given** the API returns `data.items.length === 0` for `date=2026-09-27, startTime=10:00, duration=2`, **when** the page renders, **then** the empty card shows the heading `Tidak ada lapangan yang cocok` and the body `Tidak ada lapangan kosong pada Sabtu, 27 September 2026 jam 10:00 selama 2 jam. Coba geser jam mulai atau pilih tanggal lain.`. (FR-016)
