...
it to a `const`. Two-time literals stay legal because renaming a pair rarely
earns its diff.
...
inline. `className` strings in components repeat legitimately for shared
utility classes; if the same class string appears in three components,
extract it to a `const` right there in the file, not into a shared bag.
